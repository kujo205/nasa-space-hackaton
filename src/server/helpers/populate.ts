...
export const populate = async () => {
  // const map: any = {};
  await db.deleteFrom("path_metadata").execute();
  await db.deleteFrom("path").execute();

  const promises = [];
  for (let path = 1; path <= 233; path++) {
    const promise = db.insertInto("path").values({ path }).execute();
    promises.push(promise);
  }

  await Promise.all(promises);

  for (const [landsat, landsatData] of Object.entries(data)) {
    for (const [date, dateData] of Object.entries(landsatData)) {
      const paths = dateData.path.split(",");
      for (const path of paths) {
        // map[landsat] = map[landsat] || {};
        // map[landsat][path] = map[landsat][path] || [];
        // map[landsat][path].push({
        //   date,
        //   cycle: dateData.cycle,
        // });

        db.insertInto("path_metadata")
          .values({
            date: date,
            cycle: parseInt(dateData.cycle),
            path: path,
            satellite: landsat,
          })
          .execute();
